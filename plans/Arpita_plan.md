...
> **Depends on**: Hrittika completing all 14 entities and 9 enums (Step 1.2 + 1.3 of Hrittika_plan.md). You need entity field names and enum types to define matching DTO shapes.
...
> **Depends on**: Hrittika completing `ICaseRepository`, `AppDbContext`, repository implementations, and seed data (Steps 1.4, 1.6, 1.7, 1.12 of Hrittika_plan.md).
...
> **Depends on**: Hrittika completing entities + Shads completing `AiOrchestrationService` (Step 2.4 of Shads_plan.md). The generator needs the AI-produced rights explanation and cited sections to assemble the document.
...
> **Depends on**: Steps 2.2 + 2.3 (DocumentGenerator must exist) + Hrittika's repositories.
...
> **Depends on**: Hrittika completing `LawyerProfile` entity, `User` entity, and repositories.
...
> **Depends on**: Hrittika completing repositories + seed data (Cases must exist for aggregation to be meaningful).
   ...
   public class AdminAnalyticsService
   {
       private readonly AppDbContext _context;
        ...
        public async Task<AnalyticsSummaryDto> GetSummaryAsync()
        {
            ...
            // NOTE: table names follow the repo naming rule (Hrittika_plan.md Step 1.6):
...
> **Depends on**: Hrittika completing repositories.
...
### A-F.1: Static Assets — CSS, JS, Fonts

> **Depends on**: Nothing — unblocks A-2.5 (your own `PdfExportService` needs the same font).

1. `wwwroot/css/site.css` — global styles: base body font (`Noto Sans Bengali`, 16px minimum),
   mobile-first card spacing, case-status badge classes (`badge-submitted`, `badge-underreview`,
   `badge-finalized`, `badge-rejected`), `.document-preview` box (`white-space: pre-wrap`,
   monospace-ish, `#f8f9fa` background), redline diff classes (`.content-draft`, `.content-final`).
2. `wwwroot/js/site.js` — vanilla JS: language-toggle handler (pairs with Hrittika's H-F.1),
   simple form validation helpers, a character counter for the case description textarea.
3. `wwwroot/fonts/` — download Noto Sans Bengali from Google Fonts, place
   `NotoSansBengali-Regular.ttf` / `-Bold.ttf` here. Load from the Google Fonts CDN as primary
   (in the `<link>` Shads adds to `_Layout.cshtml`), self-hosted via `@font-face` in `site.css`
   as an offline fallback — target users have intermittent connectivity. The same font file is
   what your own `PdfExportService` (A-2.5) embeds for PDF rendering.

### A-F.2: Citizen Case Intake View (`/Case/Submit`)

> **Depends on**: A-2.1 (your own `CaseService`), S-F.1.

1. `CaseSubmitViewModel` — `CategoryId`, `DistrictId`, `Title`, `Description`, `Language`,
   `IsAnonymous`, plus `Categories`/`Districts` select-list data.
2. `Views/Case/Submit.cshtml` — category/district dropdowns, title, description textarea with a
   character counter (5000 char limit), an anonymous-submission checkbox.
3. Wire `CaseController.Submit()` (GET populates dropdowns, POST) directly to
   `CaseService.SubmitCaseAsync()`. Anonymous submissions return an `AnonymousTrackingCode` —
   show it once, since the Track page (A-F.3) accepts it as credentials for that case.

### A-F.3: Anonymous Case Tracking View (`/Case/Track`)

> **Depends on**: A-2.1 (`CaseService.GetUserCasesAsync()` / tracking-code lookup), S-F.1.

`Views/Case/Track.cshtml` — table/card list of the citizen's cases with status badges
(Submitted/UnderReview/Finalized) and links to the result view. Wire to `CaseService` directly.

### A-F.4: Generated Document Preview View (`/Document/Preview`, `/Document/Download`)

> **Depends on**: A-2.4 (`DocumentService`), S-F.1.

1. `Views/Document/Preview.cshtml` — standalone document preview page (same `.document-preview`
   styling as the Case Result view).
2. Wire `DocumentController.Preview(int id)` to `DocumentService.GetDocumentAsync()`, and
   `Download(int id)` to `PdfExportService.GeneratePdf()` once A-2.5 is done — until then, keep
   the download action returning a "PDF download requires lawyer approval" message for documents
   that aren't yet approved.

### A-F.5: Lawyer Verification & Review Views (`/Lawyer/Apply`, `/Lawyer/Queue`, `/Lawyer/Review`)

> **Depends on**: A-2.6 (`LawyerVerificationService`), A-2.7 (`LawyerReviewService`), S-F.1.

1. `Views/Lawyer/Apply.cshtml` — bar registration number + specialization form, wired to
   `LawyerVerificationService.ApplyAsync()`.
2. `Views/Lawyer/Queue.cshtml` — table of documents awaiting review (Case ID, Category, Date,
   Status, Review button), wired to `LawyerReviewService.GetReviewQueueAsync()`.
3. `Views/Lawyer/Review.cshtml` — read-only AI draft panel + a review form (Decision:
   Approved/EditedApproved/Rejected, editable content shown only for EditedApproved, required
   comments), wired to `LawyerReviewService.SubmitReviewAsync()`.
4. Also cover `Views/Admin/Lawyers.cshtml` here (verification queue: bar number, status,
   approve/reject buttons) — same backend service, same owner.

### A-F.6: Admin Dashboard & Analytics View (`/Admin/Analytics`)

> **Depends on**: A-3.2 (`AdminAnalyticsService`), S-F.1.

`Views/Admin/Dashboard.cshtml` — total cases / pending reviews / approved documents as big-number
cards, cases-by-category, cases-by-district (top 10). Wire directly to
`AdminAnalyticsService.GetSummaryAsync()`. Gate with `[Authorize(Roles = "Admin")]`.

### A-F.7: Final Self-Check

> **Depends on**: All of the above.

Click through Case Submit, Case Track, Document Preview/Download, Lawyer Apply/Queue/Review, and
Admin Dashboard against real data end-to-end — no `MockData.*` calls left in your controllers.
...
|---|---|---|---|
| **1.1** All DTOs | All 14 entities + 9 enums | **Hrittika** | Step 1.2 + 1.3 (entities and enums) |
| **2.1** CaseService | `ICaseRepository` + `AppDbContext` + repositories | **Hrittika** | Steps 1.4, 1.6, 1.12 (interfaces, DbContext, repo implementations) |
...
| **2.4** DocumentService | `DocumentGenerator` (her own Step 2.2) + Hrittika's repos | **Hrittika** | Repository implementations |
| **2.5** PdfExportService | Bangla font file in `wwwroot/fonts/` | Self | Task A-F.1 (your own — place the Noto Sans Bengali font in `wwwroot/fonts/` first) |
| **2.6** LawyerVerificationService | `LawyerProfile` entity + repository | **Hrittika** | Entity + repo |
| **2.7** LawyerReviewService | Steps 2.4 + 2.6 (DocumentService + LawyerVerificationService — her own prior steps) | Self | — |
| **3.4** AdminAnalyticsService | Cases must exist in DB (seed data or test data) | **Hrittika** | Seed data loaders |

### What Arpita Can Start Immediately (After Hrittika's Entities Land)
...
While waiting for Hrittika's entities (Day 1-2):
...
Once Hrittika's PR lands (Day 2-3):
