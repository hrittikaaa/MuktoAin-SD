...
> **Prerequisite**: Shads has completed Part 1 of `Shads_plan.md` — the GitHub repo exists,
> `.gitignore` and `.editorconfig` are committed, and `appsettings.Development.json.template` is
> present in the repo.
...
|---|---|---|
| Shads | `shads/identity-ai-core` | Auth, Gemini client, encryption, AI logging, layout/home/identity/case-result views |
| Hrittika | `hrittika/schema-data-pipeline` | MSSQL schema scripts, SQL repos, batch ingestion, Qdrant/FTS search, disclaimer/search/category views |
| Arpita | `arpita/document-pipeline` | Case DTOs, document templates, QuestPDF, review flow, static assets/case/document/lawyer views |
...
4. **Execute Full-Text Search Script**:
   ...
   - Press **F5** (Execute).
   - This creates the Full-Text Catalog `ftCatalog_MuktoAin` and the Full-Text Index on `ACT_SECTION(SectionText)` (the `ActTitle` column lives on the `ACT` table, so it is filtered via a join at query time — see `Hrittika_plan.md` Step 1.10).
...
| Requirement | Hrittika | Shads | Arpita |
|---|---|---|---|
| .NET 8 SDK | ✅ Day 1 | ✅ Day 1 | ✅ Day 1 |
| SQL Server + SSMS | ✅ Day 1 | ✅ Day 1 | ✅ Day 1 |
...
| Docker Desktop | ❌ (CP3 only) | ❌ (CP3 only) | ❌ (CP3 only) |
