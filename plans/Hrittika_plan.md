# Hrittika — Data Foundation & Search Infrastructure Plan

> ponytail: full — every step earns its place. No boilerplate tasks. Options are real choices.

**Role:** Hrittika owns everything from raw schema to searchable index — the 14 entities, the data import pipeline, and the retrieval infrastructure that other teammates build on top of. She is the critical path: Shads and Arpita are both blocked until her entities and interfaces land.

---

## Part 0: Initial Repository Setup (Repo Owner Duties)

The `MuktoAin` repo lives under your GitHub account, so these two steps are yours, not Shads's —
GitHub only lets the account/repo owner set branch protection. Do these before anyone else
starts, then move on to your own Step 1.1 below.

### H-Setup.1: Create the GitHub Repository — ✅ Done

`MuktoAin` created on GitHub. `.agent/` spec folder, `AGENTS.md`, `CLAUDE.md`, `.gitignore`,
`.editorconfig`, `README.md`, and the plan docs committed as the initial commit.

### H-Setup.2: Set Up Branch Protection

> **Depends on**: H-Setup.1.
...
3. Core methods:
   ...
   public class QdrantVectorStore : IVectorStore
   {
        ...
        // after Hrittika's import PR merges).
...
FR-18 requires admin management of scenario mappings. You build the admin view (`Views/Admin/ScenarioMappings.cshtml`, task H-F.4) directly against this service.
...
### H-F.1: Disclaimer Banner + Language Toggle

> **Depends on**: Nothing — unblocks S-F.1 (the layout partial-renders these).

1. `Views/Shared/_DisclaimerBanner.cshtml` — surface 1 of 3 in the disclaimer policy.
   Non-dismissible, shown on every page:
   ```html
   ...
   ```
   CSS: `background-color: #fff3cd; border-bottom: 2px solid #ffc107; padding: 8px 0; position: sticky; top: 0; z-index: 1030;`
   No close button, no `display: none`, no JS to hide it — this is a project requirement.
2. `Views/Shared/_LanguageToggle.cshtml`:
   ```html
   ...
   ```
   Wire it in `site.js` with a simple `data-lang` click handler that toggles a CSS class and
   swaps `[data-bn]`/`[data-en]` element visibility. This is a client-side class swap, not a full
   i18n framework — Shads's `RequestLocalizationMiddleware` (S-3.5) handles server-side
   localization later via `CookieRequestCultureProvider`.

### H-F.2: Acts Keyword Search View (`/Search`)

> **Depends on**: H-2.4 (`SearchService`), S-F.1.

1. `SearchViewModel` — `Query`, `Page`, `TotalResults`, `List<SearchResultItemViewModel>`
   (`ActTitle`, `SectionNumber`, `SectionTextSnippet`).
2. `Views/Search/Index.cshtml` — search box + results list with snippets and pagination.
3. Wire `SearchController.Index(string? q, int page)` directly to
   `SearchService.SearchActsAsync()`.

### H-F.3: Category Views (`/Category`)

> **Depends on**: H-2.5 (`CategoryService`), S-F.1.

1. `Views/Category/Index.cshtml` — card grid of legal categories with descriptions.
2. `Views/Category/Details.cshtml` — single category with a "Submit a case in this category" CTA.
3. Wire `CategoryController` to `CategoryService`.

### H-F.4: Admin Acts Management & Scenario Mapping Views (`/Admin/Acts`)

> **Depends on**: H-3.1 (`ActsManagementService`), H-3.2 (`ScenarioMappingService`), S-F.1.

1. `Views/Admin/Acts.cshtml` — Acts corpus management: list of Acts, last-imported date,
   re-index button.
2. `Views/Admin/ScenarioMappings.cshtml` — table of `MappingId`, `ScenarioKeyword`, linked
   `ActTitle` + `SectionNumber`, `Notes`; add-mapping form (`SectionId`, keyword, notes) +
   delete button per row.
3. Wire `AdminController.Acts()` / `.ScenarioMappings()` (+ Add/Delete actions) directly to
   `ActsManagementService` / `ScenarioMappingService`. Gate with `[Authorize(Roles = "Admin")]`.

### H-F.5: Final Self-Check

> **Depends on**: All of the above.

Click through Search, Category, and Admin Acts/Scenario views against real data end-to-end — no
`MockData.*` calls left in your controllers.

---

## Dependency Map

Every task Hrittika can't start until someone else delivers something:

| Hrittika's Task | Blocked By | Teammate | Their Specific Task |
|---|---|---|---|
...
That's it. **Hrittika has only 2 external dependencies.** Almost everything she builds depends only on her own prior steps.

### What Hrittika Can Start Immediately (After Shads Creates the Repo)
...
Hrittika's work is almost entirely self-contained. The critical insight: **she IS the critical path.** Everyone else is waiting on her entities and interfaces. Prioritize:

1. **Day 1-2**: Steps 1.1 → 1.5 (solution, enums, entities, interfaces). PR this immediately — Shads and Arpita are both blocked.
