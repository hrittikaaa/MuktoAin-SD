...
public class LawyerControllerTests
{
    ...
    public LawyerControllerTests()
    {
        ...
        _profileRepo.Setup(r => r.GetAllAsync()).ReturnsAsync(() => new List<LawyerProfile> { _profile });
        ...
        _orderRepo.Setup(r => r.GetAllAsync()).ReturnsAsync(new List<PaymentOrder>());
        _payoutRepo.Setup(r => r.GetAllAsync()).ReturnsAsync(new List<PayoutRequest>());
    ...
    private LawyerController NewController(string? userId)
    {
        ...
        var verificationService = new LawyerVerificationService(
            _profileRepo.Object, Mock.Of<IAdminAuditService>(), notifications);
        ...
        return new LawyerController(reviewService, verificationService, paymentService, _profileRepo.Object, _userManager.Object)
        {
    ...
    public async Task Queue_ListsUnderReviewDocumentsOldestFirst()
    {
        ...
        _docRepo.Setup(r => r.GetAllAsync()).ReturnsAsync(new List<GeneratedDocument> { newer, draft, older });
    ...
    public async Task Queue_DocumentClaimedByMe_IsMineAndOpenable()
    {
        ...
        _docRepo.Setup(r => r.GetAllAsync()).ReturnsAsync(new List<GeneratedDocument> { mine });
    ...
    public async Task Queue_DocumentClaimedByAnotherLawyer_CannotBeOpened()
    {
        ...
        _docRepo.Setup(r => r.GetAllAsync()).ReturnsAsync(new List<GeneratedDocument> { theirs });
    ...
    public async Task Queue_UnclaimedFilter_HidesClaimedDocuments()
    {
        ...
        _docRepo.Setup(r => r.GetAllAsync()).ReturnsAsync(new List<GeneratedDocument> { open, claimed });
    ...
    public async Task Queue_MyFieldWithUnusableSpecialization_FallsBackToFullPool()
    {
        ...
        _docRepo.Setup(r => r.GetAllAsync()).ReturnsAsync(new List<GeneratedDocument> { doc });
    ...
    [Fact]
    public async Task Review_UnknownDocument_ReturnsNotFound()
    {
        Assert.IsType<NotFoundResult>(await _controller.Review(404));
    }
    ...
    public async Task Review_BuildsWorkspaceFromOriginalDraft()
    {
        ...
        Assert.Equal("Dhaka", view.ViewData["DistrictName"]);
        Assert.Equal("Employer withheld pay.", view.ViewData["CitizenNarrative"]);
        Assert.Equal(false, view.ViewData["CitizenEdited"]);
    }
    ...
    public async Task Review_CitizenEditedDraft_PrefillsEditorWithCitizenVersion()
    {
        ...
        Assert.Equal(3, view.ViewData["VersionNo"]);
    }
    ...
    public async Task Review_IncludesCitedSections()
    {
        ...
        var citations = Assert.IsAssignableFrom<IReadOnlyList<CitedSectionDto>>(view.ViewData["Citations"]);
    ...
    [Fact]
    public async Task SubmitReview_UnknownDecision_DefaultsToApproveWithEdits()
    {
        AddCase(1);
        var doc = AddDocument(10, 1, assignedTo: ProfileId);
        ...
        Assert.Equal("Edited by lawyer", doc.ContentFinal);
        _reviewRepo.Verify(r => r.AddAsync(It.Is<LawyerReview>(rv => rv.Decision == ReviewDecision.EditedApproved)), Times.Once);
    }

    [Fact]
    public async Task SubmitReview_ApproveWithEditsButNoText_IsRejected()
    {
    ...
    {
        SetUpHistory(
            (1, 10, "Before", ReviewDecision.Approved, new DateTime(2026, 8, 31, 23, 0, 0)),
            (2, 11, "Start", ReviewDecision.Approved, new DateTime(2026, 9, 1, 0, 0, 0)),
            (3, 12, "End", ReviewDecision.Approved, new DateTime(2026, 9, 5, 23, 59, 0)),
            (4, 13, "After", ReviewDecision.Approved, new DateTime(2026, 9, 6, 0, 1, 0)));
