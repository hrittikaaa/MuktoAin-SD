...
1. **Check Prerequisite Dependencies:** Before starting any implementation task, consult `plans/Dependency_plan.md` to verify that all preceding dependencies (`[Blocked by: ...]`) are marked completed (`[x]`).
2. **Auto-Update on Completion:** The moment you implement, verify, or complete ANY task defined in the execution plans:
   - You **MUST automatically update `plans/Dependency_plan.md`**.
...
3. **No Unrecorded Work:** Never end a conversation or complete a prompt without recording your completed task progress in `plans/Dependency_plan.md`.
